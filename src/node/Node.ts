...
export class Node extends TypedEventEmitter<NodeEvents> {
	...
	public constructor(manager: Shoukaku, options: NodeOption) {
		super();
	...
	public async connect(): Promise<void> {
		...
		for (this.reconnects = 0; this.reconnects < this.manager.options.reconnectTries; this.reconnects++) {
			try {
				...
				this.emit(
					"debug",
					`[Socket] -> [${this.name}] : Reconnecting in ${this.manager.options.reconnectInterval} seconds. ${this.manager.options.reconnectTries - this.reconnects} tries left`,
				);
				connectError = error as Error;
				await wait(this.manager.options.reconnectInterval * 1_000);
			}
		}
	...
	private async message(message: unknown): Promise<void> {
		const json:
			...
			| TrackStartEvent
			| TrackStuckEvent
			| WebSocketClosedEvent = JSON.parse(message as string);
		if (!json) {
			return;
		}
		...
		switch (json.op) {
			...
			case OpCodes.READY: {
				...
				if (!json.sessionId) {
					this.emit(
						"debug",
						`[Socket] -> [${this.name}] : No session id found from ready op? disconnecting and reconnecting to avoid issues`,
					);
					this.disconnect(1_000);
					return;
				}
	...
	private async movePlayers(): Promise<number> {
		...
		const data = await Promise.allSettled(players.map(async (player) => player.move()));
